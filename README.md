...
**验收边界：**42 项自动测试和临时 PostgreSQL 集成验证通过，Docker 构建通过，Web 登录/提交/报告通过真实浏览器验证。站点 DOM 和 iOS 前 100 名已实页核对；Python 独立采集浏览器仍需 Mac mini 登录后执行验收。云端 HTTPS、真实管理员会话和模型调用已验收；Mac 节点现场采集仍待完成。
