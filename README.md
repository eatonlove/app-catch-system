...
已部署到 [catch.meidaquan.com](https://catch.meidaquan.com)，千问 `deepseek-v4-flash-0731` 已实测连通。Mac mini 节点待现场登录与采集验收。详见 [正式部署记录](docs/PRODUCTION.md)。
...
**验收边界：**46 项自动测试和临时 PostgreSQL 集成验证通过，Docker 构建通过，Web 登录/提交/报告通过真实浏览器验证。站点 DOM 和 iOS 前 100 名已实页核对；Python 独立采集浏览器仍需 Mac mini 登录后执行验收。云端 HTTPS、真实管理员会话和模型调用已验收；Mac 节点现场采集仍待完成。
...
- [数据详情、独立报告与七个AI研究方向](docs/研究方向与报告.md)
...
云端已提供独立机会池、七因子解释评分与固定快照、资质双轴人工审核、证据/评论/检索库、CSV/JSON导入与日指标历史、E0–E4实验结果及贡献计算。13个提示词方向可使用采集数据与补充证据。详见[使用说明](docs/方案落地与使用说明-v4.md)和[未来待补充能力与数据](docs/未来待补充能力与数据.md)。缺数据的自动能力明确列出，不合成竞品、不宣称模型分数是成功率。
