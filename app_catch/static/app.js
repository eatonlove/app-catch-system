...
let researchSources=[],supplementalSources=[],previewSequence=0;
...
async function api(path,method='GET',body){const r=await fetch(path,{method,credentials:'same-origin',headers:{'Content-Type':'application/json','X-Requested-With':'appcatch'},body:body===undefined?undefined:JSON.stringify(body)});const data=await r.json();if(!r.ok){if(r.status===401)locked();throw new Error(typeof data.detail==='string'?data.detail:Array.isArray(data.detail)?data.detail.map(x=>x.loc.slice(1).join('.')+'：'+x.msg).join('；'):'请求参数不符合接口要求')}return data}
...
function showPage(id,title){document.querySelectorAll('.page').forEach(p=>p.hidden=p.id!==id);$('breadcrumb').textContent=title;document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('selected',b.dataset.page===id))}
...
async function refresh(){
 ...
 try{
  ...
  if(j.length)table($('jobs'),['入口 / 日期','状态','尝试','操作'],j.map(x=>[x.payload.recipe+' · '+x.payload.context.data_date,stateLabel(x.state)+(x.error_code?' / '+x.error_code:''),x.attempts,actions(link('查看数据','/collections/'+x.id),...(['FAILED','PARTIAL','CANCELLED'].includes(x.state)?[button('另建重试',async()=>{await api('/api/jobs/'+x.id+'/retry','POST');await refreshLater()})]:[]),...(['PENDING','RUNNING'].includes(x.state)?[button('取消',async()=>{await api('/api/jobs/'+x.id+'/cancel','POST');await refreshLater()})]:[]))]));else empty($('jobs'),'还没有采集任务。先在「采集计划」选择一个入口。');
...
async function reportDetail(id){
 ...
 if(r.model){out.append(el('h2','待验证机会线索'),el('p','模型 '+r.model.model+' · 实际输入 '+(r.model_evidence_ids?.length??'未知')+' 条证据 · '+(r.model.cached?'复用缓存':'本次生成')));
  ...
  r.model.result.candidates.forEach((c,i)=>{const card=el('article',undefined,'card');card.append(el('h3',(i+1)+'. '+c.task),el('div',c.hypothesis,'prose'),el('h4','尚待验证'),el('p',c.unknowns.join('\n'),'prose'));const refs=el('div',undefined,'evidence-list');c.evidence_ids.forEach(ref=>{const a=el('a','证据 '+ref);a.href='#evidence-'+encodeURIComponent(ref);refs.append(a,el('br'))});card.append(refs,button('保存到机会池',()=>window.captureOpportunity(id,i)));out.append(card)})
 ...
 (r.evidence||[]).forEach(e=>{const d=disclosure(e.id,e.text);d.id='evidence-'+e.id;const job=e.id.split(':')[0];d.append(job.startsWith('manual-')?link('查看补充证据库','/#data-lab',true):link('查看本次采集数据','/collections/'+encodeURIComponent(job),true));out.append(d)});
}
...
document.querySelectorAll('nav button').forEach(b=>b.onclick=()=>{if(route){location.href='/#'+b.dataset.page;return}history.replaceState(null,'','/#'+b.dataset.page);showPage(b.dataset.page,b.textContent);if(window.openLabPage)window.openLabPage(b.dataset.page).catch(e=>notice(e.message));document.querySelectorAll('nav button').forEach(x=>x.classList.toggle('selected',x===b))});
...
function researchInput(){return {use_model:$('use-model').value==='1',country:$('country').value,store:'',direction:$('direction').value||'comprehensive',guidance:$('guidance').value,evidence_limit:Number($('evidence-limit').value),evidence_record_ids:[...document.querySelectorAll('.supplemental-check:checked')].filter(x=>!x.disabled).map(x=>x.value),source_ids:[...document.querySelectorAll('.source-check:checked')].filter(x=>!x.disabled).map(x=>x.value)}}
async function loadSources(){[researchSources,supplementalSources]=await Promise.all([api('/api/research-sources'),api('/api/evidence')]);drawSources();drawSupplemental();await previewScope()}
...
function renderScope(root,s,preview=false){
 ...
 const grid=el('div');table(grid,['数据来源 / 日期','记录数',preview?'AI将读取数':'AI读取数','核对'],s.sources.map(x=>[scope(x.context),x.row_count,s.model_inputs===null?'历史未记录':s.model_inputs.filter(e=>e.id.startsWith(x.id+':')).length,link(x.evidence_id?'查看证据库':'查看批次',x.evidence_id?'/#data-lab':'/collections/'+x.id,true)]));root.append(grid);
...
$('reload-sources').onclick=()=>loadSources().catch(e=>notice(e.message));$('country').onchange=()=>{drawSources();drawSupplemental();previewScope()};$('evidence-limit').onchange=previewScope;
...
function drawSupplemental(){const root=$('supplemental-list');root.replaceChildren();if(!supplementalSources.length){root.append(el('p','暂无补充证据。'));return}for(const s of supplementalSources){const label=el('label'),box=el('input');box.type='checkbox';box.value=s.id;box.className='supplemental-check';box.checked=(new URLSearchParams(location.search).get('sources')||'').split(',').includes('manual-'+s.id);box.disabled=!!$('country').value&&$('country').value!==s.data.country;box.onchange=previewScope;label.append(box,el('span',s.data.title+' · '+s.data.country+'/'+s.data.store+' · '+s.data.observed_at));root.append(label)}}
